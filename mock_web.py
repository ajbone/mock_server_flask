...
def searchproject():
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT DISTINCT(project_name) from mock_config where project_name <> '' or project_name is NULL")
        re= cur.fetchall()
        conn.close()
    except:
        return jsonify({'msg': "fail", "remark": "select data fail"})
...
if __name__=="__main__":
    app.run(host='0.0.0.0',debug=True,threaded=True,port=8090)
